...
当前 MJCF (`MAGICBOTZ1.xml`) 只有一个 `<geom type="plane">`，完全平坦。 以下是根据 `z1_5phase_plan.yaml` 各阶段地形配置，MuJoCo 对应的设置方案：
**** ````**** ````**** ````**** ``````**** ``````**** ``````
...
**p3 地形参数映射：** - `horizontal_scale: 0.1` → MuJoCo 每个 hfield 格子 ~0.1m - `vertical_scale: 0.005` → 最大高度变化 ~5mm（gentle） - `difficulty_range: [0.0, 0.5]` → hfield 振幅约 0-2.5mm
...
Generated from training_log_2026-05-09.md