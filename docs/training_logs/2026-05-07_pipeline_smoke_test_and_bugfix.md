...
- Config 生成 — `generate_env_config()` + `generate_ppo_override()`
- Swap config — 备份并替换 `velocity_env_cfg.py`
- Launch training — `max_iterations=50, num_envs=64`
- Wait run dir — 等待 Isaac Sim 创建 run directory（最多 5 分钟） 
- Wait checkpoint — 轮询直到至少 1 个 `model_*.pt` 出现（最多 3 分钟） 
- Stop training — `graceful_stop()`
- 标记通过 → advance 到下一个 sub-phase 
...
- `generate_env_config()` 从 `_ACTIVE_CFG_REL` 读取（活动配置路径） 
- 替换 #1 的正则 `r'^COBBLESTONE_ROAD_CFG = .*?^(\n)'` 需要空行标记 COBBLESTONE_ROAD_CFG 块的结束 
- 替换 #1 消耗了末尾的空行（作为匹配的一部分） 
- 下次生成时（如 p1_fine → p2_coarse），没有空行 → 正则一路匹配到 RobotSceneCfg 之后的空行 → 整个 `RobotSceneCfg` 类被吃掉 
- **p2-p5 全部受影响**，因为模板在每个 sub-phase 持续损坏 
...
Generated from 2026-05-07_pipeline_smoke_test_and_bugfix.md