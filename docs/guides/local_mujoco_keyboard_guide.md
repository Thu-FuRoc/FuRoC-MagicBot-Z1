...
> 使用方向键而非 WASD，避免与 MuJoCo Viewer 内置的相机/灯光快捷键冲突。 速度范围与训练一致: `lin_vel_x=[-0.5, 1.0]`, `lin_vel_y=[-0.5, 0.5]`, `ang_vel_z=[-0.5, 0.5]`
...
**左侧面板** — 渲染与仿真控制： - 渲染选项（线框、阴影、透明度等） - 仿真速度控制 - 几何体组开关（显示/隐藏碰撞体、关节轴等）
...
Generated from local_mujoco_keyboard_guide.md