...
coarse → fine 的一般规律： - `track_lin_vel_xy`: 1.5 → 2.0（更强速度跟踪） - `alive`: 0.5 → 0.15（减少存活奖励，逼迫学习） - `base_height`: -5 → -10（更强高度惩罚） - `flat_orientation_l2`: -3 → -7（更严格姿态） - `action_rate_l1`: -0.04 → -0.06（更平滑动作） - `stand_still`: 0 → -3.5（fine 阶段惩罚不动） - `undesired_contacts`: 0 → -2.0（fine 阶段惩罚碰撞） - `feet_slide`: 0 → -0.5（fine 阶段惩罚滑脚）
Generated from phase_params.md