...
> 推荐方式：直接使用 `D:/Desktop_Files/GPU-Train/RTX6000/rtx_record_video.sh`。当前一键流程会在 RTX 远端录制 Isaac Lab + MuJoCo，然后自动 `scp` 回本地、自动打标签、并删除本地 raw 视频。
...
- 自动选择空闲 GPU 
- 确认 JIT export 存在（不存在则自动导出） 
- 远程录制 Isaac Lab 视频（~15-20 min，需空闲 GPU 的 cuda:0） 
- 等待 Kit 进程清理（避免 KVDB 锁冲突） 
- 远程录制 MuJoCo 视频（~2 min） 
- `scp` 下载视频 + params 到本地 `videos/p///`
- 自动打标签、删除 raw 副本 
...
> 注意：`--disable_fabric` 是 headless 渲染必需的参数（禁用 PhysX Fabric 接口）。`--video` 会自动启用 camera 和 USD instanceable 修复。

视频输出：`/videos/play/rl-video-step-0.mp4`
...
Generated from video_recording.md — updated 2026-05-22