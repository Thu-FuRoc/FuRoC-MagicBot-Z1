...
Orchestrator 在启动训练前会做两件事： 1. 生成 `velocity_env_cfg.py`（地形 + reward 权重）→ 覆盖到 `source/.../velocity_env_cfg.py` 2. 生成 `ppo_override_cfg.py`（PPO 超参）→ 通过 `--agent_cfg` 传给训练脚本
...
关键字段： - `COBBLESTONE_ROAD_CFG` — 地形生成器配置 - `rewards` dict — 各 reward 权重 - `commands` — 速度指令范围
...
Generated from config_generation.md